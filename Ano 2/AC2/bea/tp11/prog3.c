...
void eeprom_action(char c) 
{
	...
	switch(c)
	{
		...
		case 's':
			...
			head = eeprom_readData(1);
			for (i = 0; i < savedValues; i++) 
			{
				printInt10(eeprom_readData(2+ ((currentHead+i)%maxSavedValues)));
				putChar(' ');
			}
			break;
	}
...
void _int_(4) isr_T1(void) // timer T1 
{
	...
	if (registerValues && iterationsCounter >= 1) //60
	{
		eeprom_writeData(2+((currentHead+savedValues) % maxSavedValues), (char)temperature);

		// save list's head		
		if (savedValues >= maxSavedValues) 
		{
			currentHead = (currentHead+1) % maxSavedValues;
			eeprom_writeData(1, currentHead);
		}
		else
			savedValues++;
		
		eeprom_writeData(0, savedValues % maxSavedValues);
