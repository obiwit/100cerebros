...
int producer(unsigned int id, unsigned int niter)
{
    ...
    for (i = 0; i < niter; i++)
    {
        ...
        queueSend(id, value);
        // mq_open(3); this function returns a message queue descriptor (mqd_t)
...
int main(int argc, char *argv[])
{
    ...
    {
        if ((cpid[i] = proc_create(consumer, i+1, niter)) < 0)
        {
    ...
    for (unsigned int i = 0; i < nproc; i++)
    {
        if ((ppid[i] = proc_create(producer, i+1, niter)) < 0)
        {
