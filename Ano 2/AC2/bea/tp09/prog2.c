...
void configInterrupts() {
	...
	IEC0bits.U1EIE = 1; 	// enable interrupts (ERRORs)
}
...
void _int_(24) isr_uart1(void)
{
	...
	if (IFS0bits.U1EIF == 1) 
	{
		...
		if (U1STAbits.FERR || U1STAbits.PERR) {
			char never_mind = U1RXREG; 		// read UxRXREG (to discard the character)
		}
	...
	if (IFS0bits.U1RXIF == 1) 
	{
		...
		while (U1STAbits.URXDA != 0)
		{
			...
			if (rxb.count < BUF_SIZE) 		// if reception buffer is not full
				rxb.count++;
			else {							// else increment "head" variable (discard oldest character) 
