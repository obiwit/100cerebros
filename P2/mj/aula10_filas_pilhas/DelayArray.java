...
public class DelayArray<E> {
  ...
  public DelayArray(int size, E init) {
    assert size >= 0;
    //criar array de size elementos de tipo E (tem truque!)
    buffer = (E[]) (new Object[size]);

    //encher buffer com size amostras com o valor "init"
   ...
   */
  public void in(E x) {
    // colocar x na posição next e incrementar next,
    // e "dar a volta" se next == length!
  ...
  public E get(int t) {
    assert -size() <= t && t < 0;
    // devolver amostra da posição (next+t), mas "dar a volta" se < 0!
	if((next+t) < 0)
		return buffer[0];
	return buffer[next+t+buffer.length];
