...
public class P105
{
  ...
  {
    if (args.length != 1) {
      err.println("Uso: P105 <Max-Number>");
      exit(1);
    }
    int maxNumber = Integer.valueOf(args[0]);

    BlockArrayInt a = new BlockArrayInt(50,1); // BlockArrayInt(int blockSize,int numBlocks)
    ...
    for(int n = 2;n <= maxNumber;n++) {
      if (ePrimo(n)) {
        ...
        a.put(n,size); //put(int elem,int index)
        size++;
      }
    }

    for(int i = 0;i < size;i++) {
      out.printf("%8d",a.get(i)); //get(int index)
      if ((i+1) % 10 == 0)
        out.println();
    }
    out.println();
