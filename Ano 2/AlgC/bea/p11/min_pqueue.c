...
/***** Fila com Prioridade orientada aos mínimos Nome: min_pqueue.c *****/
...
int PQueueInsert (PtPQueue ppqueue, int pelem)
{
  ...
  /* adjusting the heap, descending the fathers bigger than the element */
...
int PQueueDeleteMin (PtPQueue ppqueue, int *pelem)
{
  ...
  *pelem = ppqueue->Heap[0];  /* retirar a raiz do monte - copies the maximum element from the heap */
  ...
  /* adjusting the heap, ascending the sons smaller than the element */
  for (i = 0; i*2+1 <= ppqueue->NumElem; i = Son)
  {
    Son = 2*i+1;  /* primeiro filho - position of the first son */

    /* determinar o menor dos filhos - evaluating the smaller son */
    if (Son < ppqueue->NumElem && ppqueue->Heap[Son] > ppqueue->Heap[Son+1])
      Son++;

    /* subir o menor filho se menor que o último elemento - ascending the smaller son if smaller than the last element */
...
int PQueueIncrease (PtPQueue ppqueue, int pelem, int pvalue)
{
  ...
  {   
    /* subir os filhos maiores do que o novo valor do elemento adjusting the heap,
     ascending the sons bigger than the new value of the element */
